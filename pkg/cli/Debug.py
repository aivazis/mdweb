...
# declaration
class Debug(mdweb.shells.command, family='mdweb.cli.debug'):
    """
    ...
    def nfs(self, plexus, **kwds):
        ...
        """
        # make a channel
        channel = journal.info("mdweb.cli.nfs")
        # set up the indentation level
        indent = " " * 2

        # get the prefix
        prefix = "mdweb" if self.root is None else self.root
        # and the name server
    ...
    def vfs(self, plexus, **kwds):
        ...
        prefix = mdweb.primitives.path(
            "/mdweb" if self.root is None else self.root)
        ...
        # make a channel
        channel = journal.info("mdweb.cli.vfs")
        # sign in
