...
# declaration
class Plexus(pyre.plexus, family='mdweb.shells.plexus'):
    """
    ...
    def pyre_banner(self):
        ...
        """
        # the project header
        yield from textwrap.dedent(mdweb.meta.banner).splitlines()
        # the doc string
        yield from self.pyre_showSummary(indent='')
        # the authors
        yield from textwrap.dedent(mdweb.meta.authors).splitlines()
        # all done
        return
    ...
    def pyre_interactiveSessionContext(self, context=None):
        ...
        context['mdweb'] = mdweb  # my package
        # and chain up
    ...
    def pyre_mountApplicationFolders(self, pfs, prefix, **kwds):
        ...
        # if all goes well and we reach the intended folder without errors
            ...
            self._ux = mdweb.ux.dispatcher(plexus=self, docroot=docroot, pfs=pfs)
